  ...
  async connect() {
    ...
    for (const config of connectionConfigs) {
      try {
        ...
        console.log(
          `✅ ${config.type} connection established to ${config.url}`,
        );
        ...
        this.connected = true;
        console.log(
          `🎉 SurrealDB connected successfully via ${config.type} to ${config.url}`,
        );
        ...
        console.error(
          `❌ ${config.type} connection failed to ${config.url}:`,
          errorMessage,
        );
    ...
    console.error(
      "🔍 Server is running but client connection failed. Possible causes:",
    );
    console.error(
      "1. SurrealDB.js version compatibility issue - try: npm install surrealdb.js@^1.0.0",
    );
    ...
    const finalError = new Error(
      `All connection attempts failed. Server is running, but client connection failed. Check browser console for detailed errors. Last error: ${errors[errors.length - 1] || "Unknown"}`,
    );
    throw finalError;
  }
